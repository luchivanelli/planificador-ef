...
function datosDeClase(datos: ClaseInput) {
  ...
  return {
    ...
    temaClase: aTextoONull(datos.temaClase),
    contenidosClase: aTextoONull(datos.contenidosClase),
  ...
  const { unidadDidacticaId } = await db.$transaction(async (tx) => {
    ...
    const nueva = await tx.claseDiaria.create({
      data: {
        ...
        temaClase: original.temaClase,
        contenidosClase: original.contenidosClase,
